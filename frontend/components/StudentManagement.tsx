...
import { Student, StudentCategory, Batch, User } from '../types';
...
const StudentManagement: React.FC<StudentManagementProps> = ({ onStudentsUpdate, user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', age: 15, contact: '', category: StudentCategory.BATSMAN, joinDate: new Date().toISOString().split('T')[0], batchId: '' });
      onStudentsUpdate();
    } catch (e: any) {
      alert(e.message || "Operation failed. Check administrative status.");
    }
  };
            ...
            {students
             ...
             .map(s => (
              ...
              <tr key={s.id} className="hover:bg-blue-50/30 transition-colors group">
                ...
                <td className="p-8">
                  <span className="text-sm font-bold text-slate-600">{s.contact || 'N/A'}</span>
                </td>
                <td className="p-8">
                ...
                <td className="p-8 text-slate-500 font-black text-xs uppercase tracking-widest">
          ...
          <div className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <input type="text" value={formData.contact} onChange={e => setFormData({...formData, contact: e.target.value})} className="w-full border-2 border-slate-100 rounded-2xl p-4 font-bold outline-none focus:border-[#252968]" placeholder="+91..." />
              </div>
              ...
              <div className="space-y-2">
                ...
                <select value={formData.batchId} onChange={e => setFormData({...formData, batchId: e.target.value})} className="w-full border-2 border-slate-100 rounded-2xl p-4 font-bold outline-none focus:border-[#252968] bg-white">
                  <option value="">{editingStudent ? 'Unassigned' : 'Auto-Assign (Based on Age)'}</option>
